...
import { ClientResponse, ClientType } from "@/helpers/interface/adminTypes/adminTypes";
...
const AdminClientTable = (): JSX.Element => {
    ...
    const getClient = useQuery<ClientResponse>({
        queryKey: ['client'],
        queryFn: async () => {
            const response = await axios.get(`${baseUrl}api/v1/user/clients/for-admin/list?page=0&size=10`, config);
    ...
    const deleteClient = useMutation({
        mutationKey: ['deleteClient', selected],
        mutationFn: async () => {
    ...
    const client: ClientType = getClient.data?.data?.object;
    ...
    return (
        <div className="w-full h-[27rem] border p-5 bg-gray-50">
            <table className="w-full border-2 border-black">
                ...
                <tbody>
                    {client ? client.map((client: ClientType) => (
                        <tr key={client.id}>
                            <td className="w-1/6 border-2 border-black p-2">{client.firstName}</td>
                            <td className="w-1/6 border-2 border-black p-2">{client.lastName}</td>
                            <td className="w-1/6 border-2 border-black p-2">{client.phoneNumber}</td>
                            <td className="border-2 border-black flex justify-around py-2">
                                <button onClick={() => (setSelected(client.id), openDelete())} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
                                    Delete
                                </button>
                        ...
                        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <div className="pt-5">
                                ...
                                <div className="bg-gray-50 flex justify-end p-3 rounded-b-md w-full">
                                    ...
                                    <button onClick={() => deleteClient.mutate()} data-modal-hide="popup-modal" type="button" className="py-2.5 px-5 ms-3 text-sm font-medium text-white focus:outline-none bg-red-600 rounded-lg border border-red-200 hover:bg-red-700 hover:text-white focus:z-10 focus:ring-4 focus:ring-red-100 dark:focus:ring-red-700 dark:bg-red-800 dark:text-red-400 dark:border-red-600 dark:hover:text-white dark:hover:bg-red-700">Delete</button>
                                </div>
                            </div>
                        </div>
                    </div>
